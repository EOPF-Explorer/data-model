...
!!! tip "Opening Zarr assets — no URL parsing required"
    Every STAC asset href pointing to a Zarr group (e.g. `s3://bucket/data.zarr/measurements/reflectance`) **is itself a valid Zarr store** and can be opened directly by any Zarr-compatible client. No reverse-parsing or store-root extraction is needed. See [STAC Integration and Zarr URL Resolution](architecture.md#stac-integration-and-zarr-url-resolution) in the Architecture docs for the full model, why asset hrefs point to the multiscales root, the role of the `rel: store` link, and consolidated metadata requirements.
